...
void solve_jacobi_thread(float** matrix, int matrix_size, float* right_hand_side);
...
int main() {
	...
	while (user_choice == 1)
	{
		...
		while (run_mode_choice != 2) {
			printf("Choose: Serial Mode -> 0, CPP Thread Mode -> 1, Exit -> 2 :\nYour Choice: \n");
			...
			case 1:
			{
				...
				//omp_set_num_threads(NUM_OF_THREADS);
				solve_jacobi_thread(matrix, matrix_size, right_hand_side);
...
void solve_jacobi_thread(float** matrix, int matrix_size, float* right_hand_side) {
	...
	for (int i = 0; i < ITERATION_LIMIT; i++) {
		...
		for (int thread_i = 0; thread_i < THREAD_NUM; ++thread_i)
			...
			for (int j = thread_i; j < matrix_size; j += THREAD_NUM) {
				float sigma_value = 0;
				for (int k = 0; k < matrix_size; k++) {
					if (j != k) {
						sigma_value += matrix[j][k] * solution[k];
